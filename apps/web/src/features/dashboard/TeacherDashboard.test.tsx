...
function baseMock(overrides: Record<string, unknown> = {}) {
  mockedApiRequest.mockImplementation(
    async (path: string, opts?: { query?: Record<string, unknown> }) => {
      ...
      if (path === "/api/v1/class-arms/arm2/results")
        return mathResults("PUBLISHED");
...
describe("TeacherDashboard", () => {
  it("renders 'Classes I teach' and 'Subjects' metric cards computed as a union/count from /me/teaching, plus the Enter-grades card", async () => {
    ...
    // v0.7.2 — retargeted at the new dedicated Grades page (sidebar), not
    // the generic Classes list.
    expect(
      screen.getByRole("link", { name: "Enter grades →" }),
    ).toHaveAttribute("href", "/grades");
  });

  // v0.7.2 — the per-row "Enter grades · Enter exam scores" links moved off
  // MyClassesView (and this dashboard) onto the new dedicated Grades page;
  // MyClassesView here keeps its class-teacher-of cards + a plain
  // subject/class list only.
  it("embeds MyClassesView's existing content: class-teacher-of cards + a plain subject/class list, no per-row action links", async () => {
    authStore.setTokens({
      accessToken: "access-token",
      refreshToken: "refresh-token",
    });
  ...
  it("no evaluations/exams yet shows the empty state, not a crash", async () => {
    ...
    expect(
      await screen.findByText("No evaluations or exams created yet."),
    ).toBeInTheDocument();
  });
});
