...
const EVALUATIONS_CLOSED_LOCKED: EvaluationsListResponse = {
  ...EVALUATIONS_OPEN,
  termClosed: true,
  locked: true,
  unlockReason: null,
...
function mockCommon(
  ...
  evaluations = EVALUATIONS_OPEN,
) {
  mockedApiRequest.mockImplementation(async (path: string) => {
    if (path === "/api/v1/auth/me")
      return role === "SCHOOL_ADMIN" ? ADMIN_USER : TEACHER_USER;
...
describe("ClassGradesPage — Enter scores tab", () => {
  it("SCHOOL_ADMIN: shows the locked class+subject context, the evaluation list (not a dropdown), and loads the grid on selection", async () => {
    ...
    expect(screen.getByRole("tab", { name: "Enter scores" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
    ...
    expect(
      await screen.findByText("No students enrolled in this class."),
    ).toBeInTheDocument();
  });
  ...
  it("switching the track tab to Exams shows the exam list instead of evaluations", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "Term 1 Exam" }),
    ).toBeInTheDocument();
    expect(
  ...
  it("selecting an evaluation shows its name + class/subject context in the grid heading ('Now grading')", async () => {
    ...
    expect(
      screen.getByText(/JSS 1 A · Mathematics/, { selector: "span" }),
    ).toBeInTheDocument();
  ...
  it("selecting an exam shows its name + class/subject context in the grid heading", async () => {
    ...
    renderPage("/grades/arms/arm1?tab=enter&subjectId=sub1&track=exams");
  ...
  it("no evaluations yet: shows the named empty state, not a bare/blank list", async () => {
    ...
    expect(
      await screen.findByText("No evaluations yet — create one."),
    ).toBeInTheDocument();
  });
  ...
  it("a subject not in this class arm falls back to the generic 'Subject' label, matching the old locked-context behavior", async () => {
    mockedApiRequest.mockImplementation(async (path: string) => {
      ...
      if (path === "/api/v1/grades/evaluations")
        return { ...EVALUATIONS_OPEN, subjectId: "sub2" };
      if (path === "/api/v1/sessions") return SESSIONS;
      if (path === "/api/v1/terms") return TERMS;
      throw new Error(`unexpected apiRequest call: ${path}`);
    });
    renderPage("/grades/arms/arm1?tab=enter&subjectId=sub2&track=evaluations");
...
describe("ClassGradesPage — Results tab", () => {
  ...
  // v0.7.2 step 2 (SPEC_V0.7.2.md §3) — the static placeholder became a
  // real clickable subject list, reusing armDetail.data.subjectTeachers
  // (already fetched, no new query). This is what makes the admin path
  // work now that ClassArmDetailPage no longer carries per-subject
  // grading links at all.
  it("tab=enter with no subjectId (e.g. a hand-edited URL, or GradesLandingPage's admin class-browser) shows a clickable subject list, not a dead end", async () => {
    mockCommon("SCHOOL_ADMIN");
    renderPage("/grades/arms/arm1?tab=enter");

    expect(
      await screen.findByRole("tab", { name: "Enter scores" }),
    ).toHaveAttribute("aria-selected", "true");
    expect(
      screen.getByText("Pick a subject to enter scores"),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "Enter grades" })).toHaveAttribute(
      "href",
      "/grades/arms/arm1?tab=enter&subjectId=sub1&track=evaluations",
    );
    expect(
  ...
  it("TEACHER: Results tab shows the fixed current term, no term dropdown", async () => {
    mockCommon("TEACHER");
    renderPage("/grades/arms/arm1?tab=results");
  ...
  it("switching from Results back to Enter scores via the tab bar re-shows the subject context", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByText("JSS 1 A · Mathematics"),
      ).not.toBeInTheDocument(),
    );
    await screen.findAllByText("Mathematics");
