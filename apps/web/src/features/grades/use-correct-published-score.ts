...
import type { SaveGradesGridResponse } from "@scholametric/shared";
...
// SPEC_V0.5.1.md §2.5, v0.5.1 step 4 — reuses PUT /grades/grid directly
// (the exact same reviewed write path the score-entry grid itself uses,
// just a single-item scores[]), not a new endpoint. SCHOOL_ADMIN/
// PROPRIETOR pass the PUBLISHED-student gate there; the result stays
// PUBLISHED with corrected total/grade/position (GradesService.saveGrid).
export function useCorrectPublishedScore() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: CorrectPublishedScoreInput) =>
      apiRequest<SaveGradesGridResponse>("/api/v1/grades/grid", {
        method: "PUT",
        body: {
