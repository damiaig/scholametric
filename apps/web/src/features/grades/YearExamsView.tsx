      ...
      {data.terms.length === 0 && (
        <Card>
          <CardContent className="p-10 text-center">
            <p className="text-sm text-muted">No terms yet this session.</p>
          </CardContent>
        </Card>
      )}

      {data.terms.map((term) => (
        <Card key={term.termId} className="break-inside-avoid shadow-none">
          <CardContent className="p-3">
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <p className="font-medium text-text">
                {formatTermName(term.termName)}
              </p>
              {term.status && (
                <StatusBadge
                  label={resultStatusLabel(term.status)}
                  tone={resultStatusTone(term.status)}
                />
              )}
            </div>
            ...
            {term.subjects.length === 0 ? (
              <p className="text-sm text-muted">
                No exams published for this term yet.
              </p>
            ) : (
              <div className="flex flex-col gap-2">
                {term.subjects.map((subject) => (
                  <div
                    ...
                    className="rounded-md border border-secondary/20 bg-secondary/5 p-2"
                  >
                    ...
                    <p className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted">
                      <span>
                        Subject average:{" "}
                        <span className="font-mono text-text">
                          {subject.subjectExamAverage === null
                            ? "—"
                            : formatScore(subject.subjectExamAverage)}
                        </span>{" "}
            ...
            <p className="mt-2 flex flex-wrap items-center gap-2 text-sm text-text">
              <span>
                ...
                <span className="font-mono">
                  {term.termExamAverage === null
                    ? "—"
                    : formatScore(term.termExamAverage)}
                </span>{" "}
                {term.termExamGrade ?? "—"}{" "}
                <span className="text-muted">
                  {positionLabel(term.termExamPosition)}
                </span>
              </span>
          ...
          {data.overallExamAverage === null ? (
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm">
              <p className="text-text">
                {data.overallExamGrade ?? "—"}{" "}
                <span className="text-xs text-muted">
                  ({formatScore(data.overallExamAverage)})
                </span>
              </p>
              <p className="text-muted">
                {positionLabel(data.yearExamPosition)}
              </p>
